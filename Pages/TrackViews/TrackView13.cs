	...
	public override void DrawMask(VGAMem vgaMem, Point position, MaskFields mask, int cursorPos, int fg, int bg)
	{
		var buf = new string(
			new char[]
			{
				...
				MASK_CHAR(MaskFields.Note, 0, 1, mask, cursorPos),
				'\x8F',
				MASK_CHAR(MaskFields.Instrument, 2, -1, mask, cursorPos),
				MASK_CHAR(MaskFields.Instrument, 3, -1, mask, cursorPos),
				'\x8F',
				MASK_CHAR(MaskFields.Volume, 4, -1, mask, cursorPos),
				MASK_CHAR(MaskFields.Volume, 5, -1, mask, cursorPos),
				'\x8F',
				MASK_CHAR(MaskFields.Effect, 6, -1, mask, cursorPos),
				MASK_CHAR(MaskFields.Effect, 7, -1, mask, cursorPos),
				MASK_CHAR(MaskFields.Effect, 8, -1, mask, cursorPos),
			});
