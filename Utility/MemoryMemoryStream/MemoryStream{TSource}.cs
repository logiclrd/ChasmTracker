...
namespace ChasmTracker.Utility.MemoryMemoryStream
{
			...
			where TSource : struct, ISpanOwner
	{
		...
		public sealed override long Length
		{
			...
			get
			{
				MemoryStreamFactory.ValidateDisposed(this.disposed);
		...
		public sealed override long Position
		{
			...
			{
				MemoryStreamFactory.ValidateDisposed(this.disposed);
				MemoryStreamFactory.ValidatePosition(value, this.source.Length);
		...
		public sealed override long Seek(long offset, SeekOrigin origin)
		{
			...
			long index = origin switch
			{
				...
				_ => throw new ArgumentException("Unrecognized seek origin " + origin)
			};

			MemoryStreamFactory.ValidatePosition(index, this.source.Length);
		...
		{
			MemoryStreamFactory.ValidateDisposed(this.disposed);
			MemoryStreamFactory.ValidateBuffer(buffer, offset, count);
		...
		{
			MemoryStreamFactory.ValidateDisposed(this.disposed);
			MemoryStreamFactory.ValidateCanWrite(CanWrite);
			MemoryStreamFactory.ValidateBuffer(buffer, offset, count);
